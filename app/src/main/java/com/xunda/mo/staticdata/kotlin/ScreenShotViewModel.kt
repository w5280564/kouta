...
class ScreenShotViewModel : ViewModel() {
   ...
   private var contentObserver: ContentObserver? = null
    ...
    fun getLatestImage() {
        Thread {
            try {
                var data: ScreentShotInfo?
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                    data = queryImagesP()
                } else {
                    data = queryImages()
                }
                val imagePath = data.path?.lowercase(Locale.getDefault())
                screenShoot.forEach {
     ...
     * 只获取普通图片，不获取Gif
     */
    fun queryImages(): ScreentShotInfo {
        ...
        try {
            val data = MyApplication.getInstance().contentResolver.query(
                uri,
    ...
    @WorkerThread
    fun queryImagesP(): ScreentShotInfo {
        val screentShotInfo = ScreentShotInfo()
        val uri = MediaStore.Files.getContentUri("external")
