                ...
                if (item.getType() == EMConversation.EMConversationType.GroupChat) {
                    ...
                    try {
                        if (item.getAllMsgCount() != 0) {
                            Glide.with(mContext).load(item.getLastMessage().getStringAttribute("groupHead")).into(avatar);
                            name.setText(item.getLastMessage().getStringAttribute("groupName"));
                        }
                    ...
                    try {
                        if (item.getAllMsgCount() != 0) {
                            Glide.with(mContext).load(item.getLastMessage().getStringAttribute("sendHead")).into(avatar);
                            name.setText(item.getLastMessage().getStringAttribute("sendName"));
                        }
                    } catch (HyphenateException e) {
                        e.printStackTrace();
                    }
