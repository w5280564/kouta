...
public class EaseTitleBar extends RelativeLayout implements View.OnClickListener {
    ...
    protected TextView titleView,tv_no_read;
    ...
    public void showNoReadView(int unreadMsgCount) {
        if(tv_no_read.getVisibility()==GONE) {
            tv_no_read.setVisibility(VISIBLE);
        }
        if(unreadMsgCount <= 99) {
            tv_no_read.setText(String.valueOf(unreadMsgCount));
        }else {
            tv_no_read.setText("···");
//            unread_msg_number.setBackgroundResource(R.drawable.icon_no_read_more);
        }
    }


    public void hideNoReadView() {
        if(tv_no_read.getVisibility()==VISIBLE) {
            tv_no_read.setVisibility(GONE);
        }
    }
